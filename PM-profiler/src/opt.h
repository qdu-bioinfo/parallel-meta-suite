...
//update the issue of creating the database multiple times when calling PM-profiler
...
bool pms = false; //command from pms 
...
void Parse_Para(int argc, char * argv[]) {
	...
	while(i<argc) {
		...
		switch(argv[i][1]) {
			case 'd':
				Dbseqfilename = argv[i+1];
				break;
			case 'm':
				Dbtaxafilename = argv[i+1];
				break;
			...
			case 'S':
				if ((argv[i+1][0] == 't') || (argv[i+1][0] == 'T')) Strand_mode = 1;
				break;

			case 'K':
				if (atof(argv[i+1]) > 19 || atof(argv[i+1]) < 11) {
				...
				break;
			case 'R':
				if ((argv[i+1][0] == 't') || (argv[i+1][0] == 'T')) Raw = true;
				break;
			...
			case 'Q':
				if ((argv[i+1][0] == 't') || (argv[i+1][0] == 'T')) pms = true;
				break;
