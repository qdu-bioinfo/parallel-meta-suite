	...
	switch (Step) {
		...
		case 0:
			...
			*/
			...
			//taxa
			if(profiler)
				sprintf(command, "PM-parallel-meta -M %c -l %s -o %s -t %d -f F -L %d -k %c -D %c -v %c -c %c -d %.2f -P P", Seq_type, Seq_list_file.c_str(), Singlesample_dir.c_str(), Coren, Length_t, Is_format_check, Ref_db, Is_denoised, Is_nonchimeras, db_similarity);
			else
				sprintf(command, "PM-parallel-meta -M %c -l %s -o %s -t %d -f F -L %d -k %c -D %c -v %c -c %c -d %.2f -P V", Seq_type, Seq_list_file.c_str(), Singlesample_dir.c_str(), Coren, Length_t, Is_format_check, Ref_db, Is_denoised, Is_nonchimeras, db_similarity);
			//}
