...
// version 3.7.3 - 3.7.4
// update the issue of creating the database multiple times when calling PM-profiler,Support list input
...
class _Para {
	public:
		...
		string Sample_name;
		string Sample_path;
		string Singlesample_dir;

		int Type; //0: 16S 1: shotgun
		int Format; //0: fasta 1: fastq;
		int run_mode = -1; //0: single 1: lsit
		int Length_filter;
		int Core_number;
...
int Print_Help() {
	...
	cout << "\t  -i Input a single sample file [Conflicts with -l]" << endl;
	cout << "\t  -l Input a list file for multiple samples [Conflicts with -i]" << endl;
...
int Parse_Para(int argc, char * argv[], _Para &para) { //Parse Parameters
	...
	while(i<argc) {
		...
		switch(argv[i][1]) {
			case 'D' :
				para.Database.Set_DB(argv[i+1][0]);
			...
			case 'l' :
				...
				para.Listfilename = argv[i+1];
				para.run_mode = 1;
				break; // multiple

			case 'M':
				if ((argv[i+1][0] == 'M') || (argv[i+1][0]== 'm')) {
					para.Type = 1;
					para.Is_denoised = 'F';
			...
			case 'P':
				if(argv[i+1][0] == 'P' || argv[i+1][0] == 'p') para.profiler = 1;//Profiler select,edit by Shi Haobo
				...
				*/
			case 'o' :
				para.Out_path = argv[i+1];
				para.Singlesample_dir = para.Out_path;
				break; //Default is ./result

			case 'k' :
				if ((argv[i+1][0] == 't') || (argv[i+1][0] == 'T' )) para.Is_format_check = true;
				else if ((argv[i+1][0] == 'f') || (argv[i+1][0] == 'F' )) para.Is_format_check = false;
				break;

			case 'L' :
				para.Length_filter = atoi(argv[i+1]);
				...
				break; //Default is 0
			case 't' :
				para.Core_number = atoi(argv[i+1]);
				break; //Default is Auto
			case 'f' :
				if ( (argv[i+1][0] == 'F') || (argv[i+1][0] == 'f') ) para.Is_func = false;
				break; //Default is On
			case 'h' :
				Print_Help();
				break;
			...
			case 'c' :
				...
				break;
			case 'd' :
				para.db_similarity = atof(argv[i+1]);
				break;
			default :
	...
	//check input
	if (para.Infilename.size() == 0 && para.Listfilename.size() == 0) {
		cerr << "Error: Please input the sequence file by -i or -l " << endl;
		exit(0);
	}
