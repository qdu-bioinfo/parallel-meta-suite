...
public:
    ...
    int Load_abd(_Table_Format *table, float *Abd, int sample, bool is_cp_correct, bool is_log_norm); //Load by table_format
...
int _Comp_Tree::Load_abd(_Table_Format *table, float *Abd, int sample, bool is_cp_correct, bool is_log_norm) {
    ...
    // 取log
    if(is_log_norm){
//        cout << "Log transformation applied." << endl;
        for (hash_map<string, float, std_string_hash>::iterator miter = otu_abd.begin(); miter != otu_abd.end(); miter++){
            if(miter->second <= 0){
                miter->second = 0;
            } else{
                miter->second = log(miter->second + 1);
            }
        }
        float  log_total = 0;
        for (hash_map<string, float, std_string_hash>::iterator miter = otu_abd.begin(); miter != otu_abd.end(); miter++){
            log_total += miter->second;
        }
        if(log_total > 0){
            log_total /=100.0;
            for (hash_map<string, float, std_string_hash>::iterator miter = otu_abd.begin(); miter != otu_abd.end(); miter++){
                    miter->second /= log_total;
                }
        }
    } else{
        if(total > 0){
...
int _Comp_Tree::Load_abd(_Table_Format *table, float *Abd, int sample) {
    return Load_abd(table, Abd, sample, true, false);
}
