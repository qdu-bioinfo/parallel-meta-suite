...
string Taxa_dist_name_uw = "Meta-Storms-unweighted";
string Func_dist_name = "Hierarchical-Meta-Storms";
		...
		*/
		...
		switch (Mode) {
			case 0:
				...
				break;
			case 1:
				outfile << "Input type: OTU table" << endl;
...
int Parse_TLevel(string levels) {
	...
	for(int i = 0; i < levels.size(); i ++) {
		...
		switch(levels[i]) {
			...
			default:
				cerr << "Warning: Unrec taxa level: " << levels[i] << endl;
				break;
		}
...
int Parse_FLevel(string levels) {
	...
	for(int i = 0; i < levels.size(); i ++) {
		...
		switch(levels[i]) {
			...
			default:
				cerr << "Warning: Unrec func level: " << levels[i] << endl;
				break;
		}
	...
	while(i<argc) {
		...
		switch(argv[i][1]) {
			...
			case 'D':
				...
				Ref_db = Database.Get_Id();
				break;
			case 'm':
				Meta_file = argv[i+1];
				break;
			case 'i':
				Seq_list_file = argv[i+1];
				if (Step == 1) {
			...
			case 'M':
				...
				break;
			case 'r':
				if ((argv[i+1][0] == 'F') || (argv[i+1][0]) == 'f' ) Is_cp = 'F';
				break;
			case 'a':
				Length_t = atoi(argv[i+1]);
				break;
			case 'k':
				if ((argv[i+1][0] == 't') || (argv[i+1][0] == 'T' )) Is_format_check = 'T';
				else if ((argv[i+1][0] == 'f') || (argv[i+1][0] == 'F' )) Is_format_check = 'F';
				break;

				//adv args
			case 'f':
				if ((argv[i+1][0] == 'f') || (argv[i+1][0] == 'F' )) Is_func = false;
				break;

			case 'L':
				Parse_TLevel(argv[i+1]);
				break;
			case 'F':
				Parse_FLevel(argv[i+1]);
				break;

			case 'w':
			case 'W':
				Taxa_dist_type = atoi(argv[i+1]);
				if ((Taxa_dist_type < 0) || (Taxa_dist_type > 2)) Taxa_dist_type = 2;
				break;

			case 'R':
				if ((argv[i+1][0] == 't') || (argv[i+1][0]) == 'T' ) Is_rare_curve = true;
				break;
			case 's':
			case 'S':
				Rare_depth = atoi(argv[i+1]);
				Is_rare = true;
				break;
			case 'b':
				...
				break;
			case 'E':
				if ((argv[i+1][0] == 'T') || (argv[i+1][0]) == 't' ) Is_pair = 'T';
				break;
			case 'C':
